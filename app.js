...
const HEADER_LOOKALIKE_VALUES = new Set([
  "x-ray_date", "x ray date",
  "awb_number", "awb number",
  "x-ray_image", "x ray image", "x-ray link", "xray link",
  "product_catagories", "product_category", "product category", "product catagories",
  "product_name", "product name", "product _name",
  "location"
]);

function looksLikeStrayHeaderRow(row) {
  const cells = row.slice(0, 6).map((c) => (c || "").trim().toLowerCase());
...
function rowsToProducts(rows, config) {
  // Sheet column order is: X-ray_Date(0), Awb number(1), X-ray_image(2),
  // Product_catagories(3), Product_name(4), Location(5). AWB lives in
  // column B, not column A — the Date column is legitimately blank for
  // some rows, so filtering/reading off column 0 silently corrupts or
  // drops rows. Keep this comment in sync with the sheet header order.
  const dataRows = rows
    .slice(1)
    .filter((r) => r[1] && r[1].trim())
    .filter((r) => !looksLikeStrayHeaderRow(r));

  return dataRows.map((r) => {
    const awb = (r[1] || "").trim();
    const xrayLink = (r[2] || "").trim();
    const rawCategory = normalizeCategoryKey((r[3] || "").trim().toLowerCase());
...
async function loadSidebar() {
  sidebarCategories = await getCategories();
  renderSidebar();
}
