...
const HEADER_LOOKALIKE_VALUES = new Set([
  "awb_number", "awb number", "product_name", "product name",
  "product_category", "product category", "x-ray link", "xray link"
]);

function looksLikeStrayHeaderRow(row) {
  const cells = row.slice(0, 4).map((c) => (c || "").trim().toLowerCase());
...
function rowsToProducts(rows, config) {
  ...
  return dataRows.map((r) => {
    ...
    return {
      ...
      sourceLink: xrayLink,
      driveFileId: driveFileId(xrayLink)
    };
  });
}
...
function extractAwbFromFilename(filename) {
  const match = filename.match(/[A-Z]{2}\d{8,12}[A-Z]?/i);
...
async function findExactMatchByLink(link) {
  ...
  if (!id) return { matched: false, reason: "not-a-drive-link" };
...
function showMatchResult(result) {
  ...
  if (result.reason === "no-awb-in-filename") {
    ...
    message = "That doesn't look like a Google Drive link this database can check. Paste a Drive share link, or try the search bar instead.";
