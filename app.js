...
console.log("X-ray Originals Database — Build 2026-07-19d");
...
const DEFAULT_CONFIG = {
  ...
  imageLookupUrl: "https://drive.google.com/drive/folders/1HouXx6-D024TFHV2paipHa_yVwNflRc8",
  knownBrands: [
...
// Accepts either a full Drive folder URL or a bare folder ID pasted into
// Settings, and resolves it to just the ID either way.
function resolveConfiguredFolderId(value) {
  if (!value) return null;
  const trimmed = value.trim();
  const fromUrl = driveFolderId(trimmed);
...
function fetchSheetViaJsonp(sheetId, timeoutMs = 8000) {
  return new Promise((resolve, reject) => {
    ...
    // headers=0 tells gviz not to try auto-detecting a header row itself.
    // Without it, gviz sometimes decides the header row is "data" or
    // returns blank/generic labels in table.cols[].label instead of your
    // actual header text — which silently breaks header-name matching
    // with no visible error. Forcing headers=0 guarantees the real header
    // text always lands in table.rows[0], which is what gvizResponseToRows
    // relies on below.
    const url = `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?headers=0&tqx=responseHandler:${callbackName}`;
    script.src = url;
...
// Deliberately reads header text from table.rows[0] rather than
// table.cols[].label — see the headers=0 comment above for why.
...
function buildColumnMap(headerRow) {
  ...
  for (const field of Object.keys(FALLBACK_COLUMN_POSITIONS)) {
    if (map[field] === undefined) {
      ...
      const fallbackIdx = FALLBACK_COLUMN_POSITIONS[field];
      // Only use the fallback position if it isn't already claimed by a
      // field matched by an actual header, and it's within range of this
      // sheet's column count. A collision means the fallback position is
      // almost certainly wrong here — better to leave it unresolved (-1,
      // reads as empty) than silently read another field's data.
      map[field] = (!usedIndices.has(fallbackIdx) && fallbackIdx < (headerRow || []).length)
        ? fallbackIdx
        : -1;
    }
  }
...
function rowsToProducts(rows, config) {
  ...
  return dataRows.map((r) => {
    ...
    const rowFolderId = driveFolderId(xrayLink);
    // If this row has no usable link of its own (the sheet's current
    // 3-column format — Awb number / Product_catagories / Product_name —
    // has no link column at all), fall back to the single shared Drive
    // folder configured in Settings, searched by AWB. There's still no
    // public, keyless way to resolve "the file named this AWB" into a
    // direct thumbnail without Drive API access, so this stays a one-click
    // search rather than an inline image — same honesty tradeoff as the
    // per-row folder-link case below, just applied globally.
    const configuredFolderId = resolveConfiguredFolderId(config.imageLookupUrl);
    const folderId = rowFolderId || (!fileId ? configuredFolderId : null);
...
// Only these fields actually affect what a card shows (awb, image,
// category, name). date/location are collected but never rendered
// anywhere, so their absence is cosmetic — warning about them just
// creates noise for something that isn't actually broken.
const CRITICAL_FIELDS = ["awb", "image", "category", "name"];

function renderColumnMapWarning(container) {
  const criticalUnmatched = lastColumnMapUnmatchedFields.filter((f) => CRITICAL_FIELDS.includes(f));
  if (criticalUnmatched.length === 0) return;
  const box = document.createElement("div");
  box.className = "diagnostic-banner";
  box.innerHTML = `
    <span class="diagnostic-title">Heads up: some columns were guessed</span>
    <span class="diagnostic-body">Couldn't confidently match a header for: ${escapeHtml(criticalUnmatched.join(", "))}.
    Falling back to a default column position for ${criticalUnmatched.length === 1 ? "it" : "them"}, which may be wrong if your sheet's layout changed.
    Check that the header row still reads Awb number, X-ray_image, Product_catagories, Product _name (any order is fine, the text just needs to match).</span>
  `;
...
function loadSettingsForm() {
  ...
  document.getElementById("setting-image-folder").value = config.imageLookupUrl;
...
document.getElementById("save-settings").addEventListener("click", async () => {
  ...
  const imageLookupUrl = document.getElementById("setting-image-folder").value.trim();
  ...
  saveConfig({ sheetId, imageLookupUrl, knownBrands, categoryLabels });
